...
describe('readContract', () => {
    ...
    mockSendFn = mock(async (chainId, method, params) => {
      ...
      if (method === 'eth_call') {
        ...
        if (callData === '0x20965255') { // getValue()
         ...
         if (callData === '0x38cc4831') { // getAddress() - Corrected Signature
  ...
  it('should call handler.send with correct eth_call parameters', async () => {
    await readContract({
      handler: mockHandler, chainId: testChainId, address: testAddress,
      abi: testAbi, functionName: 'getValue',
    });
    expect(mockSendFn).toHaveBeenCalledTimes(1);
    expect(mockSendFn).toHaveBeenCalledWith(testChainId, 'eth_call',
      [{ to: testAddress, data: '0x20965255' }, 'latest']
    );
  });
  ...
  it('should decode address result correctly', async () => {
    const result = await readContract<Address>({
      handler: mockHandler, chainId: testChainId, address: testAddress,
      abi: testAbi, functionName: 'getAddress',
    });
  ...
  it('should throw if function name is not in ABI', async () => {
     await expect(readContract({
      handler: mockHandler, chainId: testChainId, address: testAddress,
      abi: testAbi, functionName: 'functionNotInAbi',
   ...
   it('should throw if handler.send fails', async () => {
    ...
    it('should handle empty result ("0x") from eth_call', async () => {
        mockSendFn.mockImplementation(async (chainId, method, params) => {
         ...
         await expect(readContract({
            handler: mockHandler, chainId: testChainId, address: testAddress,
            abi: testAbi, functionName: 'getValue', // Expects uint256
    ...
    it('should fetch mocked DAI details from Gnosis (Chain 100)', async () => {
        ...
        try {
            const [symbol, totalSupply] = await Promise.all([
                 readContract<string>({
                    handler: mockHandler, chainId: gnosisChainId, address: gnosisDaiAddress,
