...
describe('RpcSelector (using Whitelist)', () => {
  ...
  beforeEach(() => {
    ...
    mockTestRpcUrlsFn = mock(async (urls: string[]): Promise<Record<string, LatencyTestResult>> => {
      ...
      urls.forEach(url => {
        ...
        else if (url.includes('1rpc.io')) { latency = Infinity; status = 'wrong_bytecode'; }
    ...
    mockCacheManager = {
      getFastestRpc: mockGetFastestRpcFn, getLatencyMap: mockGetLatencyMapFn, updateChainCache: mockUpdateChainCacheFn,
      cache: {}, cacheLoaded: true, cacheKey: 'test-cache', cacheTtlMs: 3600000,
      loadCache: mock(async () => {}), saveCache: mock(async () => {}),
    ...
    mockLatencyTester = {
      testRpcUrls: mockTestRpcUrlsFn, timeoutMs: 5000,
  ...
  it('should fallback to fastest syncing RPC if no ok RPCs are available', async () => {
    ...
    mockTestRpcUrlsFn.mockImplementationOnce(async (urls: string[]) => {
        ...
        urls.forEach(url => {
            ...
            if (url.includes('llamarpc')) { latency = 75; status = 'syncing'; }
            results[url] = { url, latency, status };
        });
        return results;
    });
    ...
    const chainId = 1;
     ...
     mockTestRpcUrlsFn.mockImplementationOnce(async (urls: string[]) => {
        ...
        urls.forEach(url => {
            let latency = Infinity;
            let status: LatencyTestStatus = 'timeout';
            if (url.includes('1rpc.io')) { latency = 20; status = 'wrong_bytecode'; } // Fastest but wrong bytecode
            results[url] = { url, latency, status };
        });
        return results;
    });
  ...
  it('should return null if all whitelisted RPCs fail latency test', async () => {
    ...
    mockTestRpcUrlsFn.mockImplementationOnce(async (urls: string[]) => {
        const results: Record<string, LatencyTestResult> = {};
        urls.forEach(url => { results[url] = { url, latency: Infinity, status: 'timeout' }; });
        return results;
    });
    const fastest = await rpcSelector.findFastestRpc(chainId);
    expect(fastest).toBeNull();
